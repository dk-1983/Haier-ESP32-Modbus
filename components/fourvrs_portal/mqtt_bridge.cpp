...
static bool valid_config(const MqttConfig &c) {
  if(c.magic!=0x484d5101 || c.enabled>1 || !c.port)return false;
...
void Portal::mqtt_setup_() {
  ...
  if(xTaskCreate(mqtt_worker_task,"haier_mqtt_io",6144,&r,1,&r.task)!=pdPASS){r.task=nullptr;r.last_error=-6;return;}
  xQueueOverwrite(r.config_queue,&mqtt_config_);
}
String Portal::mqtt_json_() {
  return String("{\"enabled\":")+(mqtt_config_.enabled?"true":"false")+",\"host\":"+json_string_(mqtt_config_.host)+
...
void Portal::mqtt_web_() {
  web_.on("/mqtt",HTTP_GET,[this](){if(!test_auth_())return;String page=FPSTR(MQTT_PAGE);page.replace("__TOKEN__",token_);
  ...
  web_.on("/mqtt/config",HTTP_POST,[this](){
    ...
    for(unsigned i=0;i<web_.args();++i){String k=web_.argName(i);if(k!="token"&&k!="enabled"&&k!="host"&&k!="port"&&k!="username"&&k!="password"&&k!="clear_password"&&k!="prefix"){web_.send(400,"text/plain","Unknown setting");return;}}
    ...
    MqttConfig next=mqtt_config_;next.enabled=en=="1";next.port=port.toInt();host.toCharArray(next.host,sizeof(next.host));user.toCharArray(next.username,sizeof(next.username));prefix.toCharArray(next.prefix,sizeof(next.prefix));
...
bool Portal::mqtt_publish_(const char *suffix,const String &body) {
  ...
  MqttPublish publish{};snprintf(publish.topic,sizeof(publish.topic),"%s/%s",mqtt_config_.prefix,suffix);body.toCharArray(publish.payload,sizeof(publish.payload));
...
void Portal::mqtt_loop_() {
  ...
  for(unsigned budget=0;budget<4 && xQueueReceive(mqtt_runtime_.rx_queue,&message,0)==pdTRUE;++budget){
    ...
    uint8_t error=0;const char *reason=nullptr;haier_bridge::Change change{};
    ...
    else error=haier_bridge::mqtt_command(field,message.payload,change);
    if(!reason && !error)error=write(&change,1);
    if(reason || error){mqtt_publish_("result",String("{\"status\":\"rejected\",\"field\":")+json_string_(field)+",\"error\":"+String(error)+",\"reason\":"+json_string_(reason?reason:"register_validation")+"}");continue;}
  ...
  if(!mqtt_last_publish_ || connections!=mqtt_seen_connections_ || uint32_t(now-mqtt_last_publish_)>=2000){
    if(mqtt_publish_("state",climate_status_())){mqtt_last_publish_=now;mqtt_seen_connections_=connections;}
  }
}
}
