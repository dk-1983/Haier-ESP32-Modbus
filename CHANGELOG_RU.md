...
- Личные пароли web, ArduinoOTA и точки настройки в NVS; защищённая страница `/settings`. Пароли сохраняются при обновлении и сбросе Wi-Fi.
- Публичная прошивка при первом запуске требует задать личные пароли до включения сетевого управления. Приватная миграция сохраняет прежние реквизиты.
- Подписанные обновления GitHub stable: HTTPS, ECDSA P-256, SHA-256, проверка платы и заголовка приложения, запись в неактивный раздел и сохранение разрешения автообновления на `/updates`.
- Ручная проверка/установка, прогресс и ошибки, исключение одновременной записи ArduinoOTA/GitHub, запрет повторов неудачного образа и программный пробный запуск/возврат.
- Готовые factory/OTA-образы и двуязычная инструкция. См. [управление и ограничения](docs/MANAGEMENT_RU.md).
