...
## Обновлённая электрическая схема — rev1.0

![Электрическая схема rev1.0](assets/haier-schematic-rev1.0.png)

[Открыть / скачать SVG](assets/haier-schematic-rev1.0.svg) · [PNG](assets/haier-schematic-rev1.0.png)

Актуальная электрическая схема будущей платы v1.0. Включены оба входных делителя RX 10/20 кОм, подтяжка GPIO21 к земле 10 кОм, терминатор R7 (120 Ом) с исходно разомкнутой перемычкой JP1, клеммник J2 (RS-485 A/B), параллельные площадки TP7/TP8, площадки UART0 для программатора и кнопки EN/BOOT. C1 — 220 мкФ / 10 В, C3 — 100 мкФ / 10 В, оба танталовые в корпусе C (6032-28). C2/C4/C5/C6 — 0,1 мкФ / 50 В. J1 — JST SM04B-GHS-TB: контакты 1–4 — +5 В, GND, TX Haier, RX Haier.

Одинаковые метки цепей означают электрическое соединение. H_TX = GPIO17 → RX Haier; H_RX = TX Haier через делитель → GPIO18. MB_TX = GPIO15 → DI MAX485; MB_RX = RO MAX485 через делитель → GPIO16; DIR = GPIO21 → DE и /RE. TP6 (+3V3) предназначена для измерения напряжения, а не отдельной подачи питания.

При сборке только для MQTT можно не устанавливать узел RS-485: U3, R4–R7, C6, JP1, J2 и TP7/TP8. Питание и делитель UART Haier сохраняются.

Проверка KiCad ERC завершилась без ошибок и предупреждений. Плата v1.0 **ещё не собрана и не проверена аппаратно**. Разводка PCB, производственные файлы и Gerber не публикуются до проверки автором. Конкретный корпус J2 ещё подбирается; электрические соединения уже определены на схеме. Ниже сохранены прежняя схема стенда и её пояснения для справки.
