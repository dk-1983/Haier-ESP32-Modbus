...
MQTT Discovery создаёт сущности, но карточку на панель нужно добавить отдельно. Отдельно устанавливать платформу Climate не требуется.

Откройте **Редактировать панель → Добавить карточку → Вручную** и вставьте:
...
`climate.haier_s3` — пример, подтверждённый в установке автора. При переименовании или наличии других устройств идентификатор может отличаться. Найдите его в **Настройки → Устройства и службы → MQTT → Haier S3**, либо откройте **Инструменты разработчика → Шаблоны (Template)** и выполните:
